angular.module('starter')
  ...
  .controller('AppCtrl', function ($rootScope, $scope, $state, $ionicConfig, $ionicModal, $ionicPopover, $filter, $ionicHistory, $window, $timeout, $http, md5, ApiEndpoint, ProductListService, CategoryListService, UserService, CartService, AUTH_EVENTS, GlobalService, LOCAL_EN, LOCAL_DE,LOCAL_FR) {
    ...
    //quand l'authentification est un succès "$scope.isAuthenticated" passe à "true"
    $scope.$on(AUTH_EVENTS.loginSuccess, function (event) {
      $scope.isAuthenticated = UserService.isAuthenticated();
    });

    //quand il change de langue, on recharge la page actuelle et on le redirige vers la page 'categories' 1 seconde plus tard 
    ...
    $scope.categories = CategoryListService.query();
...
console.log($scope.categories);
    ...
    $scope.showChangeLanguage = function () {
        //$json = JSON.parse('{"id": 3, "name": "FranÃ§ais (French)", "iso_code": "de", "test": "object:75"}');
        //GlobalService.languages.push($json);
